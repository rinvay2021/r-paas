...
const PreviewDetail: React.FC<PreviewProps> = props => {
  const { height, appCode, metaObjectCode, viewCode, onEdit, onDelete, onSetting } = props;

  const iframeSrc = appCode && metaObjectCode && viewCode
    ? `${RENDERER_ORIGIN}/?appCode=${appCode}&metaObjectCode=${metaObjectCode}&viewCode=${viewCode}`
    : null;
  ...
  return (
    ...
    >
      ...
      <div className={`${prefix}-form-content`}>
        {iframeSrc ? (
          <iframe src={iframeSrc} style={{ width: '100%', height: '100%', border: 'none', borderRadius: 8 }} title="view-preview" />
        ) : (
