...
const DetailPagePreview: React.FC<PreviewProps> = props => {
  const { height, appCode, metaObjectCode, detailPageCode, onEdit, onDelete, onSetting } = props;

  const iframeSrc = appCode && metaObjectCode && detailPageCode
    ? `${RENDERER_ORIGIN}/?appCode=${appCode}&metaObjectCode=${metaObjectCode}&detailPageCode=${detailPageCode}`
    : null;

  return (
    <div
      id="detail-page-preview"
    ...
    >
      ...
      <div className={`${prefix}-form-content`}>
        {iframeSrc ? (
          <iframe src={iframeSrc} style={{ width: '100%', height: '100%', border: 'none', borderRadius: 8 }} title="detail-preview" />
        ) : (
